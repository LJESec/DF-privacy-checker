    ...
    for filename in my_opts.files:
        if my_opts.block:
            if not my_opts.output:
                #temporär ausgeblendet  print('Calculating block local entropy on file "%s"' % filename)
	            a = ""
        else:
            if not my_opts.output:
        ...
        while buffer != '':
            if length > 0:
                ...
                if my_opts.block:
                    if not my_opts.output:
                        #temporär ausgeblendet   print('hallo1')
                        csvstring = csvstring + '%s' %(  buffer_shannon_entropy(buffer, length, probability, my_opts) ) + ';'
                        #temporär ausgeblendet   print('%s : %s' % (i, buffer_shannon_entropy(buffer, length, probability, my_opts)))
                    else:
                        #print('hallo2')
        ...
        print('' + filename + ';' + 'dummydate'+ ';' + 'size' + ';' + csvstring)
        a_file.close()
